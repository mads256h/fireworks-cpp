...
line::line(glm::vec2 start_position, glm::vec2 end_position, glm::vec3 color, float start_width,
    float end_width) noexcept : m_color(color), m_start_width(start_width), m_end_width(end_width) {
    ...
    auto rotation = std::atan2(start_position.y - end_position.y, start_position.x - end_position.x) + std::numbers::pi_v<float> / 2.0f;
